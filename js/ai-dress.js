...
$("#my_model_scene,#jp_model_scene").on("click", ".grid-item", function (e) {
  ...
  if (selectedLength) {
    ...
    //console.log(param);
    $(".select-btns").show();
    $(".select-text").text("已选“" + param.at(-1).name + "”等" + selectedLength + "张");
...
$(".generateBtn").click(function () {
  ...
  $.ajax({
    ...
    type: "POST",
    data: { bili: bili, type_id: type_id, original_image_url: original_image_url, original_image_url_2: original_image_url_2, scene_id: scene_id, image_count: image_count, auto_hand_refine: auto_hand_refine, cat_id: cat_id },
    dataType: "json",
    success: function (response) {
        if (response.state == 200) {
            ...
            $(".ai-result").show();
            $(".result-wrap").show();
            $(".result-container").html(response.msgbox)
           
            //$(".result-item .result-loading").show();
            //var taskids = response.taskids;
            setTimeout(function () {
